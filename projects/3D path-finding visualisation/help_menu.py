...
draw_opt_text = "In the 'DRAWING OPTIONS' menu, select if you want the program to draw the path, the closet set (already determined members) and the open set (members in consideration). Note that drawing any of the three\noptions will affect the performance considerably. In the 'GRID OPTIONS' menu, select the dimensions of the grid using the sliders. In the 'WALL OPTIONS' menu, select the probability of a member of the grid being\na wall. Presets can also be used to show differences between the algorithms. Note that changes into the dimensions of the grid will not be applied if a preset is used."
